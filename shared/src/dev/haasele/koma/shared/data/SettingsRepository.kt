...
import dev.haasele.koma.shared.domain.AppSettings.Companion.KEY_ACCENT_COLOR
...
class SettingsRepository(private val db: KomaDatabase) {
    ...
    suspend fun save(settings: AppSettings) = withContext(ioDispatcher) {
        db.transaction {
            ...
            queries.upsert(KEY_ACCENT_COLOR, settings.accentColor)
    ...
    private fun toSettings(values: Map<String, String>): AppSettings {
        ...
        return AppSettings(
            ...
            theme = when (val raw = values[KEY_THEME] ?: defaults.theme) {
                "system" -> "dark"
                else -> raw
            },
            accentColor = values[KEY_ACCENT_COLOR]?.takeIf { it.isNotBlank() } ?: defaults.accentColor,
