    ...
    Box(Modifier.fillMaxSize()) {
        LazyColumn(
            ...
            items(pages, key = { it.id }) { page ->
                ClickableRow(onClick = { onOpen(page.id) }, modifier = Modifier.animateItem()) {
...
fun StatusPageViewerScreen(core: KomaCore, pageId: Long, onEdit: (Long) -> Unit) {
    ...
    var beatSlots by remember { mutableStateOf(80) }
    val palette = LocalStatusPalette.current

    LaunchedEffect(pageId, beatSlots) {
        view = core.statusPageService.viewById(pageId, beatCount = beatSlots.coerceAtLeast(50))
        incidents = core.statusPages.incidents(pageId)
    }

    val model = view ?: return
    val accentTarget = palette.colorFor(model.overall)
    val accent by animateColorAsState(accentTarget, animationSpec = KomaMotion.status(), label = "status-accent")
    val overallLabel = when (model.overall) {
    ...
    LazyColumn(
        ...
        model.groups.forEach { group ->
            ...
            items(group.monitors, key = { it.monitor.id }) { entry ->
                Column(Modifier.fillMaxWidth().padding(vertical = 8.dp).animateItem()) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
