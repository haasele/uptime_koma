...
fun MonitorDetailScreen(
    ...
    var beatSlots by remember { mutableStateOf(60) }
    var daySlots by remember { mutableStateOf(30) }
    val beatLimit = maxOf(beatSlots + 8, 80)
    val beats by core.heartbeats.observeRecent(monitorId, beatLimit).collectAsState(emptyList())
    ...
    LazyColumn(
        ...
        item {
            val actions = listOf(
                MonitorAction(
                    label = if (current.active) "Pause" else "Resume",
                    icon = if (current.active) KomaIcons.Pause else Icons.Default.PlayArrow,
                    onClick = {
                        scope.launch {
        ...
        item {
            Column {
                SectionTitle("Last $beatSlots beats")
                HeartbeatBar(
                    beats = beats,
                    modifier = Modifier.fillMaxWidth(),
                    barHeight = 34,
                    active = current.active,
                    onVisibleSlotsChange = { beatSlots = it },
                )
                Spacer(Modifier.height(6.dp))
        ...
        item {
            Column {
                SectionTitle("Last $daySlots days")
                DailyUptimeChart(
                    days = daily,
                    modifier = Modifier.fillMaxWidth().height(70.dp),
                    onVisibleDaysChange = { daySlots = it },
                )
                Spacer(Modifier.height(6.dp))
                val daySummary = daily.count { it.down > 0 }.let { badDays ->
                    if (badDays == 0) "No day with a failed check" else "$badDays days with failures"
                }
                AnimatedContent(
                    targetState = daySummary,
                    transitionSpec = { KomaMotion.contentCrossfade() },
                    label = "detail-day-summary",
                ) { text ->
                    Text(
                        text = text,
                        style = MaterialTheme.typography.bodySmall,
        ...
        items(events, key = { it.id }) { event ->
            Row(Modifier.fillMaxWidth().padding(vertical = 8.dp).animateItem()) {
...
private data class MonitorAction(
    val label: String,
...
@Composable
private fun MonitorActionRow(actions: List<MonitorAction>) {
    val iconOnly = Platform.kind != PlatformKind.DESKTOP
    Row(
        Modifier.fillMaxWidth().padding(top = 14.dp),
        horizontalArrangement = Arrangement.spacedBy(if (iconOnly) 4.dp else 8.dp),
    ) {
        actions.forEach { action ->
            if (iconOnly) {
                ...
                AssistChip(
                    ...
                    leadingIcon = {
                        Icon(action.icon, contentDescription = null, modifier = Modifier.width(16.dp))
                    },
                )
            }
        }
